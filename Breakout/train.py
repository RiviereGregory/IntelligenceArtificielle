...
def train(rank, params, shared_model, optimizer):
    ...
    done = True # Variable pour dire que l'on commence le jeu
    episode_length = 0
    while True:
        episode_length += 1
        # Téléchargement des poids dans le cloud
        model.load_state_dict(shared_model.state_dict())
        # initialisation des hx et cx
        if done:
        ...
        for step in range(params.num_steps):
            ...
            # Jouer l'action calculé
            state, reward, done, _ = env.step(action.numpy())
            reward = max(min(reward, 1), -1)
            rewards.append(reward)
            # Si le jeu est finie ou si on a atteitn le max de tour
